...
export default function ChallengeSolves({ challengeId }) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await api.get(`/play/challenges/${challengeId}/solves`, {
        params,
      });

      setChallengeSolves(response.data.items);
  ...
  return (
    <>
      ...
      <Table striped bordered hover className="mt-4">
        ...
        <tbody>
          {challengeSolves === undefined ? (
            <tr>
              <td colSpan={5} className="text-center">
                Click the search button to view challenge solves.
              </td>
            </tr>
          ) : challengeSolves.length > 0 ? (
            challengeSolves.map((challengeSolve) => (
              <tr
              ...
              >
                ...
                <td>{new Date(challengeSolve.solvedAt).toLocaleString()}</td>
              </tr>
            ))
          ) : (
            <tr>
      ...
      <div className="mt-3">
        <p>Total Challenge Solves: {totalCount}</p>
        <p>
