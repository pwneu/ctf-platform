export const faq = [ 
  {
    id: 1,
    question: "What is a Capture The Flag (CTF) competition?",
    answer:
      "A CTF competition involves solving security-related challenges to capture \"flags\" and earn points. These flags are hidden in various digital environments or embedded in files. The goal is to solve these challenges to progress and score high.",
  },
  {
    id: 2,
    question: "How do I get started with CTF challenges?",
    answer:
      "Simply sign up for an account on our platform, and you'll gain access to a variety of challenges. Start by choosing challenges that match your skill level and follow the instructions provided for each challenge.",
  },
  {
    id: 3,
    question: "Do I need to have prior cybersecurity knowledge to participate?",
    answer:
      "While some challenges may require advanced knowledge, many are designed for beginners. We offer a range of difficulties to accommodate different skill levels, so you can start learning and improving your skills progressively.",
  },
  {
    id: 4,
    question: "How can I track my progress in the CTF competition?",
    answer:
      "You can monitor your progress through your user dashboard, which displays your current score, the flags you've captured, and your ranking compared to other participants.",
  },
  {
    id: 5,
    question: "What should I do if I get stuck on a challenge?",
    answer:
      "If you're stuck, try revisiting the challenge details and explore online resources or forums. Additionally, our platform may provide hints or solutions for some challenges to help guide you.",
  },
  {
    id: 6,
    question: "How often are new challenges added?",
    answer:
      "We regularly update the platform with new challenges to keep the content fresh and engaging. Check back frequently or subscribe to our newsletter for updates on new challenges and competitions.",
  },
  {
    id: 7,
    question: "Can I collaborate with others on challenges?",
    answer:
      "Collaboration is generally allowed, but please check the rules of each competition or challenge. Some CTFs may have specific guidelines about team participation or individual efforts.",
  },
  {
    id: 8,
    question: "What should I do if I encounter technical issues or bugs on the platform?",
    answer:
      "If you experience technical issues, please contact our support team via the support page or email us at support@pwneuplatform.com. Provide details about the issue, including any error messages, and we’ll work to resolve it as quickly as possible.",
  },
  {
    id: 9,
    question: "How do I reset my password if I forget it?",
    answer:
      "You can reset your password by clicking on the \"Forgot Password\" link on the login page. Follow the instructions provided to receive a password reset link via email.",
  },
  {
    id: 10,
    question: "Where can I find more resources or learning materials to improve my skills?",
    answer:
      "We offer a variety of resources, including tutorials, guides, and recommended reading materials. Check out our Resources section or visit our blog for tips and educational content related to cybersecurity and CTF challenges.",
  },
];
